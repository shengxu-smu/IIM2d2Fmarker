c-----------------------------------------------------------------------
c
      subroutine diffusion_pressure(gama)
      ...
      real*8 alfa0,alfa1,alpha,signx,signy,sx,sy,gama
      real*8 aa(6),bb(6),f(6)
     ...
     .               0.5d0*pjcyc(3,j,l)*(xc(ic)-sx)**2.0d0)
      ...
      do j=1,ny
        do i=1,nx
          funrho = rho_neg*funHp(i,j) + rho_pos*(1-funHp(i,j))
          Re4p   =  Re_neg*funHp(i,j) + Re_pos*(1-funHp(i,j))
          prhs(i,j)= funrho*(
c     .             dn(i,j)/(fac*dt)
c     .            +(dx2/Re4p)*(d(i+1,j)-2.0d0*d(i,j)+d(i-1,j))
c     .            +(dy2/Re4p)*(d(i,j+1)-2.0d0*d(i,j)+d(i,j-1))
c     .            -2.0d0*(d(i,j)*d(i,j)+
c     .             (ucc(i,j)/dx)*0.5d0*(d(i+1,j)-d(i-1,j))+
c     .             (vcc(i,j)/dy)*0.5d0*(d(i,j+1)-d(i,j-1)))
     ...
     .                                 pcdxx(2,j,l)
      ...
      call helmholtz_fft(pw,pe,ps,pn,gama,2,2,2,2,1)
